...
function ComicCategory() {
  ...
  return (
    <div className="flex flex-wrap justify-center md:justify-between px-4 md:px-24 py-4">
      <button
        className="hover:border-2 hover:border-black p-2 rounded-3xl hover:shadow-xl transform hover:-translate-x-1 hover:-translate-y-1 transition-all duration-150 mb-2 md:mb-0"
      >
      ...
      >
        ...
        {showMore && (
          <div className="absolute top-10 left-0 z-50 w-40 py-2 bg-white border-2 border-black rounded-lg shadow-md">
            {category.slice(10).map((item) => (
              <button
                key={item.key}
                className="block w-full px-4 py-2 text-gray-800 hover:bg-gray-200 z-50"
              >
                <img alt="" src={item.title} className="h-4" />
              </button>
            ))}
